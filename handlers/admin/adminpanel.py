...
@dp.callback_query_handler(IsSuperAdmin(), text='close', state='*')
...
@dp.callback_query_handler(IsSuperAdmin(), text='pause_or_resume', state='*')
...
@dp.callback_query_handler(IsSuperAdmin(), text='delete_mail', state='*')
...
@dp.callback_query_handler(IsSuperAdmin(), lambda callback_query: callback_query.data.startswith('update_mail'), state='*')
...
@dp.callback_query_handler(IsSuperAdmin(), lambda callback_query: callback_query.data.startswith('location_'), state='*')
async def mail_location_(callback_query: types.CallbackQuery,state: FSMContext):
    res = await Database.get("SELECT `send`,`not_send`,`status`,`type`,`location` FROM `mailing`")
    if res:
        send, not_send, status, type, location = res['send'], res['not_send'], res['status'], res['type'], res[
...
@dp.callback_query_handler(text=['mail_group'],user_id=ADMINS)
...
@dp.message_handler(IsSuperAdmin(), state=AdminForm.gr_mailling, content_types=types.ContentTypes.ANY)
...
@dp.message_handler(IsSuperAdmin(), state=AdminForm.mailling, content_types=types.ContentTypes.ANY)
async def mailling(message: types.Message,state: FSMContext):
    message_id = message.message_id
    chat_id = message.chat.id
    reply_markup = ''
...
@dp.callback_query_handler(IsSuperAdmin(), text='stat',state='*')
...
@dp.callback_query_handler(IsSuperAdmin(), text='channels_on_off',  state='*')
...
@dp.callback_query_handler(IsSuperAdmin(), lambda c: c.data == 'channels' or c.data == 'back_to_channels',  state='*')
...
@dp.callback_query_handler(IsSuperAdmin(),lambda c: c.data.startswith('channel_'),  state='*')
...
@dp.callback_query_handler(IsSuperAdmin(),lambda c: c.data == 'add_channel', )
...
@dp.message_handler(IsSuperAdmin(),state=FormChannel.channel_id, )
...
@dp.callback_query_handler(IsSuperAdmin(),lambda c: c.data == 'delete_channel', )
...
@dp.callback_query_handler(IsSuperAdmin(),lambda c: c.data.startswith('delete_channel_'), )
...
@dp.callback_query_handler(IsSuperAdmin(),lambda c: c.data == 'channels_list', )
...
@dp.callback_query_handler(IsSuperAdmin(),lambda c: c.data == 'ads_add', )
async def add_channel(callback_query: types.CallbackQuery):
...
@dp.message_handler(IsSuperAdmin(),state=FormAds.txt)
...
@dp.message_handler(IsSuperAdmin(),state=FormAds.link)
async def adshandl(message: types.message, state: FSMContext):
...
@dp.callback_query_handler(IsSuperAdmin(),lambda c: c.data == 'ads_remove', )
async def add_channel(callback_query: types.CallbackQuery):
    res = await Database.get_all_ads()
    keyboard = types.InlineKeyboardMarkup()
    r = []
...
@dp.callback_query_handler(IsSuperAdmin(),lambda c: c.data.startswith('rmv_ads:'), )
async def delete_channel_id(callback_query: types.CallbackQuery):
